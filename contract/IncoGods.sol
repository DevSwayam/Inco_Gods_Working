  ...
  function attackOrDefendChoice(uint8 _choice, string memory _battleName) external {
    ...
    function _resolveBattle(Battle memory _battle) internal {
        ...
        if (p1.move == 1 && p2.move == 1) {
            if (TFHE.decrypt(p1.attack) >= (p2.health)) {
                _endBattle(_battle.players[0], _battle);
            } else if (TFHE.decrypt(p2.attack) >= p1.health) {
                _endBattle(_battle.players[1], _battle);
            } else {
                players[p1.index].playerHealth -= TFHE.decrypt(p2.attack);
                players[p2.index].playerHealth -= TFHE.decrypt(p1.attack);
            ...
            uint256 PHAD = p2.health + TFHE.decrypt(p2.defense);
            if (TFHE.decrypt(p1.attack) >= PHAD) {
                _endBattle(_battle.players[0], _battle);
            } else {
                uint256 healthAfterAttack;

                if (TFHE.decrypt(p2.defense) > TFHE.decrypt(p1.attack)) {
                    healthAfterAttack = p2.health;
                } else {
                    healthAfterAttack = PHAD - TFHE.decrypt(p1.attack);
            ...
            uint256 PHAD = p1.health + TFHE.decrypt(p1.defense);
            if (TFHE.decrypt(p2.attack) >= PHAD) {
                _endBattle(_battle.players[1], _battle);
            } else {
                uint256 healthAfterAttack;

                if (TFHE.decrypt(p1.defense) > TFHE.decrypt(p2.attack)) {
                    healthAfterAttack = p1.health;
                } else {
                    healthAfterAttack = PHAD - TFHE.decrypt(p2.attack);
        ...
        euint8 _randomAttackStrengthPlayer1 = _createRandomNum();
        ...
        euint8 _randomAttackStrengthPlayer2 = _createRandomNum();
    ...
    function createSinglePlayerBattle(
        string memory _name
    ) external returns (Battle memory) {
        require(isPlayer(msg.sender), "Please Register Player First"); // Require that the player is registered
        require(!isBattle(_name), "Battle already exists!"); // Require battle with same name should not exist
    ...
    function attackOrDefendChoiceForSinglePlayerBattle(
        ...
        _registerPlayerMove(0 , _choice, _battleName);
